...
import { Copy, Edit2, ExternalLink, Info, MoreVertical, MoveRight, Share2, Trash2 } from "lucide-react";
    ...
    return (
        <>
            <DropdownMenu open={isMenuOpen} onOpenChange={setIsMenuOpen}>
                ...
                <DropdownMenuContent align="end" onClick={(e) => e.stopPropagation()} className="w-52 bg-background/80 backdrop-blur-xl border-border/50 text-foreground z-[130] shadow-2xl">
                    {folder.is_trashed ? (
                        <>
                            <DropdownMenuItem className="cursor-pointer" onClick={(e) => { e.stopPropagation(); setIsMenuOpen(false); handleRestore(); }}>
                                <ExternalLink className="mr-2 h-4 w-4" />
                                <span>Restore</span>
                            </DropdownMenuItem>
                            <DropdownMenuSeparator className="opacity-20" />
                            <DropdownMenuItem className="text-red-500 hover:bg-red-500/10 focus:bg-red-500/10 cursor-pointer" onClick={(e) => { e.stopPropagation(); setIsMenuOpen(false); handleDelete(); }}>
                        ...
                        <>
                            {canEdit && (
                                <DropdownMenuItem className="cursor-pointer" onClick={(e) => { e.stopPropagation(); setIsMenuOpen(false); setIsRenaming(true); }}>
                            ...
                            {isOwner && (
                                <>
                                    <DropdownMenuItem className="cursor-pointer" onClick={(e) => { e.stopPropagation(); setIsMenuOpen(false); setIsMoveOpen(true); }}>
                                        <MoveRight className="mr-2 h-4 w-4" />
                                        <span>Move</span>
                                    </DropdownMenuItem>
                                    <DropdownMenuItem className="cursor-pointer" onClick={(e) => { e.stopPropagation(); setIsMenuOpen(false); setIsCopyOpen(true); }}>
                            ...
                            {canManageShares && (
                                <DropdownMenuItem className="cursor-pointer" onClick={(e) => { e.stopPropagation(); setIsMenuOpen(false); setIsSharingOpen(true); }}>
                                    <Share2 className="mr-2 h-4 w-4" />
                                    <span>Share</span>
                                </DropdownMenuItem>
                            )}
                            <DropdownMenuItem className="cursor-pointer" onClick={(e) => { e.stopPropagation(); setIsMenuOpen(false); setIsDetailsOpen(true); }}>
                            ...
                            {canEdit && (
                                <>
                                    <DropdownMenuSeparator className="opacity-20" />
                                    <DropdownMenuItem className="text-red-500 hover:bg-red-500/10 focus:bg-red-500/10 cursor-pointer" onClick={(e) => { e.stopPropagation(); setIsMenuOpen(false); handleTrash(); }}>
                                        <Trash2 className="mr-2 h-4 w-4" />
            ...
            {isOwner && (
                <MoveCopyDialog
                    ...
                    submitAction={async ({ destinationFolderId }) => {
                        await moveFolder({ id: folder.id, destinationFolderId });
                        router.refresh();
                    }}
                />
            )}
            ...
            {isOwner && (
                <MoveCopyDialog
                    ...
                    loadFolders={async () => await listFolderOptions() as { folders?: any[] }}
                    submitAction={async ({ destinationFolderId }) => {
                        await copyFolder({ id: folder.id, destinationFolderId });
                        router.refresh();
                    }}
                />
            )}
